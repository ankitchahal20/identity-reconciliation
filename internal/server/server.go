...
// Registering the Identify EndPoint
func registerIdentifyEndPoints(handler gin.IRoutes) {
	handler.POST(constants.ForwardSlash+strings.Join([]string{constants.ForwardSlash, constants.Identify}, constants.ForwardSlash), service.Identify())
}

func Start() {
	plainHandler := gin.New()

	identityReconciliationHandler := plainHandler.Group(constants.ForwardSlash + constants.Version).Use(gin.Recovery()).
		Use(middleware.ValidateInputRequest())
	registerIdentifyEndPoints(identityReconciliationHandler)
